...
export const ReminderComponent = function (props: { api: Api; }) {
  ...
  const fetchReminders = async function () {
    const response = await props.api.getAllReminders()
    if (response.ok()) {
  ...
  const createReminder = async function (description: string, userIds: Array<string>, amount: number) {
    const response = await props.api.createReminder(description, userIds, amount);
    if (response.ok()) {
